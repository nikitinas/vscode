...
import { derived, derivedOpts, derivedWithStore, IObservable, IReader, observableValue, recomputeInitiallyAndOnChange } from '../../common/observable.js';
...
export abstract class ObserverNode<T extends Element = Element> {
	...
	keepUpdated(store: DisposableStore): ObserverNodeWithElement<T> {
		...
		store.add(recomputeInitiallyAndOnChange(d));
