...
import { ICodeEditor, IContentWidget, IEditorMouseEvent, IOverlayWidget, IOverlayWidgetPosition, ContentWidgetPositionPreference } from './editorBrowser.js';
...
export class ObservableCodeEditor extends Disposable {
	...
	*/
	...
	public observePosition(position: IObservable<Position | null>, store: DisposableStore): IObservable<Point | null> {
		...
		const contentWidgetId = `observablePositionWidget` + (this._overlayWidgetCounter++);
	...
	public isTargetHovered(predicate: (target: IEditorMouseEvent) => boolean, store: DisposableStore): IObservable<boolean> {
		const isHovered = observableValue('isTargetHovered', false);
		store.add(this.editor.onMouseMove(e => {
			const val = predicate(e);
			isHovered.set(val, undefined);
		}));

		store.add(this.editor.onMouseLeave(() => {
