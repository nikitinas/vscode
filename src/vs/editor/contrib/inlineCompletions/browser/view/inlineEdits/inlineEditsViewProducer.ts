...
export class InlineEditsViewAndDiffProducer extends Disposable { // TODO: This class is no longer a diff producer. Rename it or get rid of it
	...
	private readonly _inlineEdit = derived<InlineEditWithChanges | undefined>(this, (reader) => {
		...
		// In 1.96, we use inlineEditState which has inlineCompletion (InlineCompletionWithUpdatedRange)
		const inlineEditState = model.inlineEditState.read(reader);
		if (!inlineEditState) { return undefined; }

		const inlineCompletion = inlineEditState.inlineCompletion;
		const edit = inlineCompletion.toSingleTextEdit(reader);
		const diffEdits = new TextEdit([edit]);
		const text = new TextModelText(textModel);

		const commands = inlineCompletion.inlineCompletion.source.inlineCompletions.commands ?? [];
		return new InlineEditWithChanges(text, diffEdits, model.primaryPosition.read(reader), commands, inlineCompletion);
	});
