...
import { IObservable, ISettableObservable, autorun, constObservable, debouncedObservable, derived, observableValue, runOnChange } from '../../../../../../../base/common/observable.js';
...
export class InlineEditsGutterIndicator extends Disposable {
	...
	constructor(
		...
		this._isHoveredOverInlineEditDebounced = debouncedObservable(this._isHoveringOverInlineEdit, 100, this._store).map(v => v ?? false);
	...
	// Sticky scroll not available in 1.96
	private readonly _stickyScrollHeight = constObservable(0);
	...
	private readonly _layout = derived(this, reader => {
		...
		const gutterEditArea = Rect.fromRanges(new OffsetRange(gutterViewPortWithoutStickyScroll.left, gutterViewPortWithoutStickyScroll.right), verticalEditRange);
		...
		const iconNoneDocked = this._tabAction.map(action => action === InlineEditTabAction.Accept ? Codicon.chevronRight : Codicon.arrowRight);
		const iconDocked = derived(reader => {
			...
			return cursorLineNumber <= editStartLineNumber ? Codicon.chevronUp : Codicon.chevronDown;
		});
	...
	private readonly _isHoveredOverIconDebounced: IObservable<boolean> = debouncedObservable(this._isHoveredOverIcon, 100, this._store).map(v => v ?? false);
	...
	private _showHover(): void {
		...
		// Use showHover instead of showInstantHover (not available in 1.96)
		...
		}, false) as IHoverWidget | undefined;
		if (h) {
			this._hoverVisible.set(true, undefined);
			disposableStore.add(this._editorObs.editor.onDidScrollChange(() => {
				if (!h.isDisposed) {
					h.dispose();
				}
			}));
			// In 1.96, IHoverWidget doesn't have onDispose, so we track disposal manually
			const originalDispose = h.dispose.bind(h);
			h.dispose = () => {
