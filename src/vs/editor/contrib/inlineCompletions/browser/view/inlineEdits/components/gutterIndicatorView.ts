...
import { mapOutFalsy, rectToProps } from '../utils.js';
...
export class InlineEditsGutterIndicator extends Disposable {
	...
	private readonly _activeCompletionId = derived<string | undefined>(reader => {
		...
		return JSON.stringify([inlineEdit.inlineCompletion.filterText, inlineEdit.inlineCompletion.insertText, inlineEdit.inlineCompletion.range.getStartPosition().toString()]);
	});
	...
	constructor(
		...
		const debounceStore = this._register(new DisposableStore());
		this._isHoveredOverInlineEditDebounced = debouncedObservable(this._isHoveringOverInlineEdit, 100, debounceStore).map(v => v ?? false);
	...
	private readonly _stickyScrollHeight = this._stickyScrollController
		? observableFromEvent(this, this._editorObs.editor.onDidLayoutChange, () => {
			const state = this._stickyScrollController!.stickyScrollWidgetState;
			return state.startLineNumbers.length > 0 ? (state.endLineNumbers.length * this._editorObs.getOption(EditorOption.lineHeight).get()) : 0;
		})
		: constObservable(0);
	...
	private readonly _debounceStore = this._register(new DisposableStore());
	private readonly _isHoveredOverIconDebounced: IObservable<boolean | undefined> = debouncedObservable(this._isHoveredOverIcon, 100, this._debounceStore);
