...
import { Command, InlineCompletion, InlineCompletionContext, InlineCompletionEndOfLifeReasonKind, InlineCompletionTriggerKind, InlineCompletionsProvider, PartialAcceptTriggerKind } from '../../../../common/languages.js';
...
export class InlineCompletionsModel extends Disposable {
	...
	// Removed: _onlyShowWhenCloseToCursor - distance restriction has been removed to allow inline edits at any position
	...
	private _lastShownInlineCompletionInfo: { alternateTextModelVersionId: number; /* already freed! */ inlineCompletion: InlineCompletionWithUpdatedRange } | undefined = undefined;
	private _lastAcceptedInlineCompletionInfo: { textModelVersionIdAfter: number; /* already freed! */ inlineCompletion: InlineCompletionWithUpdatedRange } | undefined = undefined;
	private readonly _didUndoInlineEdits = derivedHandleChanges({
		...
		if (versionId !== null
			...
			&& this._lastAcceptedInlineCompletionInfo.inlineCompletion.inlineCompletion.sourceInlineCompletion.isInlineEdit
	...
	private readonly _fetchInlineCompletionsPromise = derivedHandleChanges({
		...
		if (context.triggerKind === InlineCompletionTriggerKind.Automatic) {
			if (this.textModel.getAlternativeVersionId() === this._lastShownInlineCompletionInfo?.alternateTextModelVersionId) {
				...
				context = {
					...context,
					includeInlineCompletions: !this._lastShownInlineCompletionInfo.inlineCompletion.inlineCompletion.sourceInlineCompletion.isInlineEdit,
					includeInlineEdits: this._lastShownInlineCompletionInfo.inlineCompletion.inlineCompletion.sourceInlineCompletion.isInlineEdit,
				};
			}
		}
	...
	public stop(stopReason: 'explicitCancel' | 'automatic' = 'automatic', tx?: ITransaction): void {
		subtransaction(tx, tx => {
			if (stopReason === 'explicitCancel') {
				...
				if (inlineCompletion) {
					// Note: reportEndOfLife may not exist in 1.96
					const completion = inlineCompletion.toInlineCompletion(undefined);
					if (completion.reportEndOfLife) {
						completion.reportEndOfLife({ kind: InlineCompletionEndOfLifeReasonKind.Rejected });
					}
				}
			}
	...
	public readonly state = derivedOpts<{
		...
		if (item?.inlineEdit) {
			...
			// Removed distance restriction: allow inline edits at any position
			// Previously: if (this._onlyShowWhenCloseToCursor.read(reader) && cursorDist > 3 && !item.inlineEdit.request.isExplicitRequest && !this._inAcceptFlow.read(reader)) {
			// 	return undefined;
			// }
			...
			const renderExplicitly = this._jumpedToId.read(reader) === item.inlineEdit.semanticId;
		...
		// Reset jumpedToId when switching away from inline edit
	...
	public readonly warning = derived(this, reader => {
		return this.inlineCompletionState.read(reader)?.inlineCompletion?.inlineCompletion.sourceInlineCompletion.warning;
	});
	...
	public readonly showCollapsed = derived<boolean>(this, reader => {
		...
		if (state.inlineCompletion.inlineCompletion.sourceInlineCompletion.displayLocation) {
			return false;
		}

		// In 1.96, we don't have updatedEditModelVersion, so we always check if collapsed mode is enabled
		const isCurrentModelVersion = true; // Assume current version for now
	...
	public readonly tabShouldAcceptInlineEdit = derived(this, reader => {
		...
		const sourceCompletion = s.inlineCompletion.inlineCompletion.sourceInlineCompletion;
		const targetRange = (sourceCompletion as any).targetRange;
		if (targetRange && targetRange.startLineNumber === this._editorObs.cursorLineNumber.read(reader)) {
			return true;
		}
	...
	public async accept(editor: ICodeEditor = this._editor): Promise<void> {
		...
		let completion: InlineCompletionWithUpdatedRange;
		...
		const inlineCompletion = completion.toInlineCompletion(undefined);
		// Note: reportEndOfLife may not exist in 1.96, so we use optional chaining
		if (inlineCompletion.reportEndOfLife) {
			inlineCompletion.reportEndOfLife({ kind: InlineCompletionEndOfLifeReasonKind.Accepted });
		}

		if (inlineCompletion.command) {
			// Make sure the completion list will not be disposed.
			completion.source.addRef();
		}

		editor.pushUndoStop();
		if (inlineCompletion.snippetInfo) {
			TextModelChangeRecorder.editWithMetadata(this._getMetadata(completion), () => {
				editor.executeEdits(
					'inlineSuggestion.accept',
					[
						EditOperation.replace(inlineCompletion.range, ''),
						...inlineCompletion.additionalTextEdits
					]
				);
			});
			editor.setPosition(inlineCompletion.snippetInfo.range.getStartPosition(), 'inlineCompletionAccept');
			SnippetController2.get(editor)?.insert(inlineCompletion.snippetInfo.snippet, { undoStopBefore: false });
		} else {
			...
			// In 1.96, displayLocation may not exist, so we always move the cursor
			const sourceCompletion = completion.inlineCompletion.sourceInlineCompletion;
			if ((sourceCompletion as any).displayLocation === undefined) {
		...
		if (inlineCompletion.command) {
			await this._commandService
				.executeCommand(inlineCompletion.command.id, ...(inlineCompletion.command.arguments || []))
	...
	private _getMetadata(completion: InlineCompletionWithUpdatedRange, type: 'word' | 'line' | undefined = undefined): ITextModelChangeRecorderMetadata {
		return {
			extensionId: completion.source.provider.groupId ?? '',
			nes: completion.inlineCompletion.sourceInlineCompletion.isInlineEdit ?? false,
			type
		};
	}
	...
	public jump(): void {
		...
		transaction(tx => {
			...
			// In 1.96, targetRange may not exist, use editRange instead
		const sourceCompletion = s.inlineCompletion.inlineCompletion.sourceInlineCompletion;
		const targetRange = (sourceCompletion as any).targetRange ?? s.inlineEdit.range;
			...
			const sourceCompletion = s.inlineCompletion.inlineCompletion.sourceInlineCompletion;
			const insertText = typeof sourceCompletion.insertText === 'string' ? sourceCompletion.insertText : sourceCompletion.insertText.snippet;
			const isSingleLineChange = targetRange.isSingleLine() && ((sourceCompletion as any).displayLocation || !insertText.includes('\n'));
	...
	public async handleInlineSuggestionShown(inlineCompletion: InlineCompletionWithUpdatedRange): Promise<void> {
		// Note: reportInlineEditShown may not exist in 1.96, so we use optional chaining
		if (inlineCompletion.reportInlineEditShown) {
